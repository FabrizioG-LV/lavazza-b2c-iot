...
class Device:
  ...
  errors_backoff_until: float | None = None

  @classmethod
  def from_list_item(
      cls, raw: dict[str, Any], catalog: ModelCatalog, app_brand: str
  ) -> "Device":
    """Create device from API list response (device_list endpoint).

    Expected fields from API: dsn, model, machineName. There's no brand
    field on the device object itself -- confirmed live, the response never
    includes one -- so app_brand is which brand-scoped list_devices() call
    this device came back under, passed by the caller, not read from raw.
    ...
    return cls(
        ...
        matricola=matricola,
        # friendlyName is the user's own name for their specific unit (e.g.
        # "TabliRed"); machineName is the generic model name, same for every
        # unit of that model -- prefer the personalized one as the device's
        # display name, matching how the vendor's own app shows it.
        name=raw.get("friendlyName") or raw.get("machineName", dsn),
        catalog=catalog,
        app_brand=app_brand,
    )
