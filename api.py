...
class ApiError(Exception):
  ...
  def __init__(self, message: str, status_code: int | None = None) -> None:
    """Initialize with an optional HTTP status code so callers can branch on it."""
...
class ApiClient:
  ...
  def __init__(
    ...
    Args:
      ...
      installation_id: This HA instance's identifier.
      client_secret: This HA instance's secret.
      broker_url: Worker base URL — used for broker-level routes (e.g. /catalogs)
        that aren't part of the vendor API endpoint map.
      broker_key: Broker access credential.
      app_version: App version string sent with requests (default V99.99.99).
    """
    self.token = token
  ...
  async def list_devices(self, app_brand: str | None = None) -> dict[str, Any]:
    """Fetch list of devices for account, scoped to one brand.

    The account-level device list is filtered server-side by the appBrand
    header -- confirmed live: the same token returns a completely different
    device under appBrand=tabli than with no header at all (which defaults
    to lavazza-only). There's no field on the device objects themselves that
    reports which brand they belong to, so callers must query once per known
    brand and merge to discover all of a user's devices.

    Args:
      app_brand: "lavazza" | "tabli" | "cartenoire". None omits the header
        (server default, returns lavazza-brand devices only).
    ...
    try:
      headers = self._with_brand_header(
          {
    ...
    try:
      ...
      async with self.session.get(
        ...
        if resp.status == 401:
          raise AuthError("Token expired or invalid")
        if resp.status >= 400:
          raise ApiError(f"get_errors failed: {resp.status}", status_code=resp.status)
        return await resp.json()
    except aiohttp.ClientError as e:
  ...
  async def get_buzzer_info(self, unit_serial: str) -> dict[str, Any]:
  ...
  async def register_fcm_token(self, fcm_token: str) -> dict[str, Any]:
    ...
    Args:
      fcm_token: Push notification registration token.
  ...
  async def get_catalogs(self, model_code: str) -> dict[str, Any]:
    ...
    A broker-level route (worker's own GET /catalogs), not a vendor-API-proxied
    one — doesn't come from the /api-config endpoints map, so it's signed and
    called against broker_url directly, same auth shape as /register or /token.
    ...
    Raises:
      AuthError: Broker key/signature rejected (401).
      ApiError: Other API errors or model not found (404).
    """
    url = f"{self.broker_url}/catalogs"
    path = "/catalogs"
    try:
      headers = {
          "X-Broker-Key": self.broker_key,
          **self._sign_request("GET", path),
      }
      async with self.session.get(
          url,
        ...
        if resp.status == 401:
          raise AuthError("Broker key or signature invalid")
        if resp.status == 404:
  ...
  def _sign_request(self, method: str, path: str, body: str = "") -> dict[str, str]:
    """Build signing headers for this request."""
