...
from .catalog import load_catalog, set_catalog_data
from .const import (
    BROKER_KEY,
    BROKER_URL,
    CONF_ACCESS_TOKEN,
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
  ...
  # device_list refreshes on its own first, before the others: it's the only
  # way to learn which model codes need catalogs (Device objects are built
  # here, and device_state/device_errors below read device_list_coordinator.data
  # -- both would race an empty/stale result if run in the same gather as
  # device_list itself).
  if device_list_coordinator:
  ...
  try:
    ...
    for device in device_list_data.values():
      if device.model_code:
        model_codes.add(device.model_code)
    ...
    for model_code in model_codes:
      try:
        ...
        # Continue — catalog.py falls back to an empty catalog on cache miss
  except Exception as e:
    _LOGGER.warning("Failed to fetch catalogs: %s", e)

  # Store catalogs in config entry and populate cache
  if catalogs:
    # entry.data is a read-only MappingProxyType -- direct item assignment
    # raises TypeError. Updates go through async_update_entry.
    hass.config_entries.async_update_entry(
        entry, data={**entry.data, CONF_CATALOGS: catalogs}
    )
    set_catalog_data(catalogs)
    _LOGGER.debug("Loaded %d catalogs from worker", len(catalogs))
    # The Device objects above were built before the cache existed (had to
    # discover the model codes before knowing which catalogs to fetch), so
    # their .catalog is still the empty fallback. Patch in place -- load_catalog
    # is lru_cache'd, this is an in-memory lookup, not another network call.
    for device in device_list_data.values():
      device.catalog = load_catalog(device.model_code)
  ...
  # Refresh the remaining coordinators now that device_list.data is populated
  ...
  # Set up push receiver for real-time device state updates
  ...
  if device_state_coordinator:
    ...
    def on_fcm_notification(parsed_payload: dict) -> None:
      """Handle push notification by updating device state coordinator.
  ...
  async def async_check_token_validity() -> None:
    ...
    try:
      ...
      _LOGGER.error("Token validity check failed: %s", e)
